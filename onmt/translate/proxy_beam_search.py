...
class ProxyBeamSearch(DecodeStrategy):
    """Generation beam search. The beam is maintained using the secondary objective. 
    ...
    def __init__(self, beam_size, batch_size, pad, bos, eos, n_best,
                 ...
                 stepwise_penalty, ratio, pos_topk=1, sec_bos=None, multi_task=True, vocab2pos=None, use_feat_emb=False):
        super(ProxyBeamSearch, self).__init__(
            pad, bos, eos, batch_size, beam_size, min_length,
            block_ngram_repeat, exclusion_tokens, return_attention,
            max_length, sec_bos=sec_bos, multi_task=multi_task, use_feat_emb=use_feat_emb)
        # beam parameters
        ...
        self.sec_hypotheses = [[] for _ in range(batch_size)]
    ...
    def initialize(self, memory_bank, src_lengths, src_map=None, device=None, pos_topk=1):
        ...
        super(ProxyBeamSearch, self).initialize(
        ...
        self.best_sec_scores = torch.full(
            [self.batch_size], -1e10, dtype=torch.float, device=device)
        ...
        self.topk_sec_log_probs = torch.tensor(
            [0.0] + [float("-inf")] * (self.beam_size - 1), device=device
        ).repeat(self.batch_size)
        # buffers for the topk scores and 'backpointer'
        self.topk_scores = torch.empty((self.batch_size, self.beam_size),
                                       dtype=torch.float, device=device)
        self.topk_sec_scores = torch.empty((self.batch_size, self.beam_size),
                                       dtype=torch.float, device=device)
        self.topk_ids = torch.empty((self.batch_size, self.beam_size),
                                    dtype=torch.long, device=device)
        self.topk_sec_ids = torch.empty((self.batch_size, self.beam_size),
                                    dtype=torch.long, device=device)
    ...
    def advance(self, log_probs, attn, sec_log_probs):
        vocab_size = log_probs.size(-1)
        sec_vocab_size = sec_log_probs.size(-1)

        # using integer division to get an integer _B without casting
        _B = sec_log_probs.shape[0] // self.beam_size
        ...
        sec_log_probs += self.topk_sec_log_probs.view(_B * self.beam_size, 1)
        ...
        curr_sec_scores = sec_log_probs / length_penalty
        ...
        curr_sec_scores = curr_sec_scores.reshape(_B, self.beam_size * sec_vocab_size)
        # print(curr_sec_scores[0])
        torch.topk(curr_sec_scores, self.beam_size, dim=-1,
                   out=(self.topk_sec_scores, self.topk_sec_ids))

        # Recover log probs.
        # Length penalty is just a scalar. It doesn't matter if it's applied
        # before or after the topk.
        torch.mul(self.topk_sec_scores, length_penalty, out=self.topk_sec_log_probs)

        # Resolve beam origin and map to batch index flat representation.
        torch.div(self.topk_sec_ids, sec_vocab_size, out=self._batch_index)
        self._batch_index += self._beam_offset[:_B].unsqueeze(1)
        self.select_indices = self._batch_index.view(_B * self.beam_size)
        self.topk_sec_ids.fmod_(sec_vocab_size)  # resolve true word ids
        ...
        sec_one_hot = torch.zeros(
                (_B * self.beam_size, sec_vocab_size)).to(log_probs.device).scatter_(
                    dim=-1, index=self.topk_sec_ids.view(_B * self.beam_size, 1), value=1.0)  # (batch_size*beam_size x sec_vocab_size)
        # for each word in the vocab, get 1 if the pos tag is predicted for that word, else 0
        activated_vocab = sec_one_hot.matmul(self.vocab2pos.t()).gt(0.).float()  # (batch_size*beam_size x V) 
        # print(self.topk_sec_ids)
        # print(activated_vocab.size())
        # input()

        topk_scores, topk_ids = torch.max(activated_vocab * log_probs.index_select(0, self.select_indices), dim=-1)
        # print(topk_ids)
        # print(log_probs.index_select(0, self.select_indices))
        # input()
        self.topk_scores = topk_scores.view(-1, self.beam_size)
        self.topk_ids = topk_ids.view(-1, self.beam_size)
        
        self.topk_log_probs = self.topk_log_probs.view(_B, self.beam_size) 
        self.topk_log_probs += self.topk_scores
        # self.topk_log_probs += self.topk_scores
        
        #compute topk_ids from topk_sec_ids
    ...
    def update_finished(self):
        ...
        self.topk_sec_log_probs.masked_fill_(self.is_finished, -1e10)
        ...
        sec_predictions = self.alive_sec_seq.view(_B_old, self.beam_size, step)
        attention = (
        ...
        for i in range(self.is_finished.size(0)):  # Batch level
            ...
            for j in finished_hyp:  # Beam level: finished beam j in batch i
                if self.ratio > 0:
                    ...
                    sec_s = self.topk_sec_scores[i, j] / (step + 1)
                    if self.best_scores[b] < s:
                        self.best_scores[b] = s
                    if self.best_sec_scores[b] < sec_s:
                        self.best_scores[b] = s 
                ...
                self.sec_hypotheses[b].append((
                    self.topk_sec_scores[i, j],
                    sec_predictions[i, j, 1:],  # Ignore start_token.
                    None))
            # End condition is the top beam finished and we can return
            # n_best hypotheses.
            if self.ratio > 0:
                pred_len = self.memory_lengths[i] * self.ratio
                finish_flag = ((self.topk_sec_scores[i, 0] / pred_len)
                               <= self.best_sec_scores[b]) or \
                    self.is_finished[i].all()
            else:
                finish_flag = self.top_beam_finished[i] != 0
            if finish_flag and len(self.hypotheses[b]) >= self.n_best:
                best_hyp, best_sec_hyp = zip(*sorted(
                    zip(self.hypotheses[b], self.sec_hypotheses[b]), key=lambda x: x[1][0], reverse=True))
                for n, (score, pred, attn) in enumerate(best_hyp):
                    if n >= self.n_best:
                        break
                    self.scores[b].append(score)
                    self.sec_scores[b].append(best_sec_hyp[n][0])
                    self.predictions[b].append(pred)  # ``(batch, n_best,)``
                    self.sec_predictions[b].append(best_sec_hyp[n][1])
        ...
        self.topk_log_probs = self.topk_log_probs.index_select(0,
                                                               non_finished)
        self.topk_sec_log_probs = self.topk_sec_log_probs.index_select(0,
                                                               non_finished)
        self._batch_index = self._batch_index.index_select(0, non_finished)
        self.select_indices = self._batch_index.view(_B_new * self.beam_size)
        self.alive_seq = predictions.index_select(0, non_finished) \
            .view(-1, self.alive_seq.size(-1))
        self.alive_sec_seq = sec_predictions.index_select(0, non_finished) \
            .view(-1, self.alive_sec_seq.size(-1))
        self.topk_sec_scores = self.topk_sec_scores.index_select(0, non_finished)
        self.topk_scores = self.topk_scores.index_select(0, non_finished)
        self.topk_sec_ids = self.topk_sec_ids.index_select(0, non_finished)
