...
class Translator(object):
    def __init__(self, opt):
        ...
        if self.opt.lookup_dict:
            self.lookup_dict = pickle.load(open(self.opt.lookup_dict,"rb"))
        else:
        ...
        if self.opt.replace_unk:
            for i in range(len(tokens)):
                if tokens[i] == onmt.Constants.UNK_WORD:
                    ...
                    if self.lookup_dict is not None and src[maxIndex[0]] in self.lookup_dict:
                        tokens[i] = self.lookup_dict[src[maxIndex[0]]]
                    else:
