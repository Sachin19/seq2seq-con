...
declare -a modelnames=("enpt.vmf") # "fren.transformer.vmf.3") #("fren.transformer.vmf.long" "fren.transformer.vmf.5" "fren.transformer.vmf.6" "fren.transformer.vmf.7") # "fren.transformer.vmf.1" "fren.transformer.vmf.2" "fren.transformer.vmf.3" "fren.transformer.vmf.4")
BS=1
...
for modelname in "${modelnames[@]}"; do
    ...
    for i in {10000..200000..10000}; do
        python -u translate.py\
            ...
            -src ../../../data/parallel/en-pt/test.tok.true.en\
            -output logs/$modelname/step_${i}_pred.bs$BS.pt\
            -batch_size 4000\
            -batch_type tokens\
            -beam_size $BS\
            -replace_unk\
            -usenew >> logs/$modelname/decodelogs.out 2>&1
        ./evaluate.sh logs/$modelname/step_${i}_pred.bs$BS.pt ../../../data/parallel/en-pt/test.pt pt >> logs/$modelname/bleus.txt
    done
done
...
for modelname in "${modelnames[@]}"; do
    ...
    for i in {10000..200000..10000}; do
        python -u translate.py\
            ...
            -gpu 0\
            -model logs/$modelname/model_step_${i}.pt\
            -src ../../../data/parallel/en-pt/wit3/test-wit-pteu.tok.true.en\
...
for modelname in "${modelnames[@]}"; do
    ...
    for i in {10000..200000..10000}; do
        python -u translate.py\
            -gpu 0\
            -model logs/$modelname/model_step_${i}.pt\
            -src ../../../data/parallel/en-pt/wit3/test-wit-ptbr.tok.true.en\
