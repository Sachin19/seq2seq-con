...
declare -a modelnames=("wmt16deen.transformer.vmf.untied") # "fren.transformer.vmf.3") #("fren.transformer.vmf.long" "fren.transformer.vmf.5" "fren.transformer.vmf.6" "fren.transformer.vmf.7") # "fren.transformer.vmf.1" "fren.transformer.vmf.2" "fren.transformer.vmf.3" "fren.transformer.vmf.4")
BS=1
for modelname in "${modelnames[@]}"; do
    echo $modelname
    rm logs/$modelname/decodelogs.out
    rm logs/$modelname/bleus-both-phrase.txt
    for i in {10000..250000..10000}; do
        python -u translate.py\
            ...
            -beam_size $BS\
            -replace_unk\
            -phrase_table ../../../data/parallel/en-de/wmt16/de-en.both-phrasetable.txt\
            -usenew >> logs/$modelname/decodelogs.out 2>&1
        ./evaluate.sh logs/$modelname/step_${i}_pred.bs$BS.en ../../../data/parallel/en-de/wmt16/newstest2016.en en >> logs/$modelname/bleus-both-phrase.txt
    done
done
