...
class Logcmk(torch.autograd.Function):
    ...
    def forward(ctx, k, m=300):
        ...
        answer = (m / 2 - 1) * torch.log(k) - torch.log(torch.from_numpy(scipy.special.ive(m / 2 - 1, k_cpu))).to(k.device) - k - (m / 2) * np.log(2*np.pi)
        answer = answer.float()
        return answer
    ...
    def backward(ctx, grad_output):
        ...
        k = k.double()
        k_cpu = k.data.cpu().numpy()
        x = -(torch.from_numpy(scipy.special.ive(m / 2, k_cpu)) / torch.from_numpy(scipy.special.ive(m / 2 - 1, k_cpu))).to(k.device)
