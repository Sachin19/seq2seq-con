"""
This script is taken from Graham Neubig's github page:
https://github.com/neubig/util-scripts/blob/master/compare-mt.py
"""
...
parser.add_argument('--ner_file', type=str, default=None, help='A link to the file containing NERs in the ref_file')
...
with open(args.ref_file, "r") as f:
  reflines = [line.strip() for line in f]
  ref = [line.split() for line in reflines]
with open(args.out_file, "r") as f:
  outlines = [line.strip() for line in f]
  out = [line.split() for line in outlines]
if args.out2_file != None:
  with open(args.out2_file, "r") as f:
    out2lines = [line.strip() for line in f]
    out2 = [line.split() for line in out2lines]
if args.ner_file is not None:
  with open(args.ner_file, "r") as f:
    ners = [line.strip() for line in f]
...
def calc_ner_matches(reflines, outlines, ner):
  matches = [0, 0, 0]
  for refline, outline in zip(reflines, outlines):
    for ne in ner:
      refne = ne in refline
      outne = ne in outline
      if refne and outne:
  ...
  if matches[0] == 0:
    ...
    rec = matches[0]/float(matches[1])
    prec = matches[0]/float(matches[2])
    fmeas = 2*prec*rec/(prec+rec)
  yield matches[0], matches[1], matches[2], rec, prec, fmeas

def calc_matches_by_freq(ref, out, buckets):
  extended_buckets = buckets
  # print (type(max(freq_counts.values())))
  extended_buckets += [max(freq_counts.values()) + 1]
...
# Analyze the differences between two systems
  ...
  matches = calc_ner_matches(reflines, outlines, ners)
  matches2 = calc_ner_matches(reflines, out2lines, ners)
  print('\n\n********************** NER Analysis ************************')
  print('--- f-measure for NER')
  for match, match2 in zip(matches, matches2):
    print(list(match), list(match2))
    print("{:.4f}\t{:.4f}".format(match[5], match2[5]))
