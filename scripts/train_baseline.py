...
parser.add_argument('-train_anew', action='store_true', help="Load from the train_from model but restart optimizer")
...
def memoryEfficientLoss(outputs, targets, generator, crit, eval=False):
    ...
    for i, (out_t, targ_t) in enumerate(zip(outputs_split, targets_split)):
        ...
        # print pred_t.size()
        num_correct_t = pred_t.data.eq(targ_t.view(-1).data).masked_select(targ_t.view(-1).ne(onmt.Constants.PAD).data).float().sum()
...
def trainModel(model, trainData, validData, dataset, optim):
    ...
    def trainEpoch(epoch):
        ...
        for i in range(len(trainData)):
            ...
            if i % opt.log_interval == -1 % opt.log_interval:
                print("Epoch %2d, %5d/%5d; acc: %6.2f; ppl: %6.2f; %3.0f src tok/s; %3.0f tgt tok/s; %3.0f sample/s; %6.0f s elapsed" %
    ...
    for epoch in range(opt.start_epoch, opt.epochs + 1):
        ...
        torch.save(checkpoint,
                   '%s_latestmodel.pt' % opt.save_model)
        torch.save(checkpoint,
                   '%s_model_%d.pt' % (opt.save_model,epoch))
...
def main():
    ...
    generator = nn.Sequential(
        nn.Linear(opt.rnn_size, dicts['tgt'].size(), bias=True),
        nn.LogSoftmax())
